...
6. The expected correlation side condition.
7. A prohibition on side effects, external provider calls, invented evidence,
...
Validate the final report. If repair is possible without changing evidence,
allow one structural repair. Otherwise return the non-passing validation state
